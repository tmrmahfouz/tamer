...
export default function AdminChatPage() {
  ...
  const deleteConversation = async (convId: string) => {
    if (!confirm('هل أنت متأكد من حذف هذه المحادثة؟ سيتم حذف جميع الرسائل أيضاً.')) return
  ...
  return (
    <AdminLayout title="إدارة الدردشة">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-between text-xs text-gray-500">
                          <span className="flex items-center gap-1">
                            ...
                            {conv.participants.length} مشاركين
                          </span>
                          <span className="flex items-center gap-1">
                    ...
                    {selectedConv.participants.map((p) => (
                      <div key={p._id} className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg">
                        ...
                        <span className={`text-xs px-2 py-1 rounded-full ${
                          ...
                          p.role === 'instructor' ? 'bg-blue-100 text-blue-700' :
