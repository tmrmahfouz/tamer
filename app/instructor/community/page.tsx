...
export default function InstructorCommunityPage() {
  ...
  const stats = {
    ...
    totalMembers: studyGroups.reduce((acc, g) => acc + (g.members?.length || 0), 0),
    totalReplies: discussions.reduce((acc, d) => acc + (d.replies?.length || 0), 0),
  }

  if (loading) {
    return (
      <InstructorLayout title="إدارة المجتمع">
          ...
          <div className="flex border-b">
            <button
              ...
              className={`flex-1 px-6 py-4 font-medium flex items-center justify-center gap-2 ${
                activeTab === 'discussions' ? 'bg-green-50 text-green-600 border-b-2 border-green-600' : 'text-gray-600'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 px-6 py-4 font-medium flex items-center justify-center gap-2 ${
                activeTab === 'groups' ? 'bg-green-50 text-green-600 border-b-2 border-green-600' : 'text-gray-600'
              }`}
            >
                      ...
                      <div className="flex items-start gap-4">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-4 mt-3 text-sm text-gray-500">
                            <span className="flex items-center gap-1">
                              <Users className="w-4 h-4" />
                              {group.members?.length || 0}/{group.maxMembers || 0} عضو
                            </span>
