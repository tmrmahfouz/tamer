...
import { StickyNote, MessageCircle, Send, Clock, User, BookOpen, FileText, ExternalLink, Paperclip, Filter, CheckCircle, AlertCircle, X, Image, Download, Trash2, Link2, Plus } from 'lucide-react'
                          ...
                          {att.type === 'file' && isImageFile(att.fileType) ? (
                            <div className="relative group cursor-pointer" onClick={() => setPreviewImage(att.url)}>
                              <img 
                                src={att.url} 
                                alt={att.name} 
                                className="w-24 h-24 object-cover rounded-lg border-2 border-gray-200 hover:border-green-400 transition-colors"
                              />
      ...
      {previewImage && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={() => setPreviewImage(null)}>
          <div className="relative max-w-4xl max-h-[90vh]">
            ...
            <a href={previewImage} download className="absolute bottom-2 right-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm flex items-center gap-2">
