...
export default function NewLessonPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.title) { setError('عنوان الدرس مطلوب'); setLoading(false); return }
    if (formData.type === 'video' && !formData.content.videoUrl) { setError('رابط الفيديو مطلوب'); setLoading(false); return }
    if (formData.type === 'pdf' && !formData.content.pdfUrl) { setError('رابط PDF مطلوب'); setLoading(false); return }
    if (formData.type === 'text' && !formData.content.textContent) { setError('المحتوى النصي مطلوب'); setLoading(false); return }
    ...
    try {
      ...
      else setError(data.message || 'حدث خطأ أثناء إضافة الدرس')
    } catch (error) {
  ...
  return (
    <InstructorLayout title="إضافة درس جديد">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-lg p-8 space-y-6">
          <div>
            ...
            <input type="text" name="title" value={formData.title} onChange={handleChange} required
              className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-green-600" placeholder="مثال: مقدمة في Python" />
          </div>

          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">الوصف</label>
            <textarea name="description" value={formData.description} onChange={handleChange} rows={3}
              className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-green-600 resize-none" placeholder="وصف مختصر للدرس..." />
          </div>
              ...
              {formData.content.videoProvider === 'upload' ? (
                ...
                <div>
                  ...
                  <input type="url" value={formData.content.videoUrl} onChange={(e) => handleContentChange('videoUrl', e.target.value)}
                    className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-green-600" placeholder="https://www.youtube.com/watch?v=..." />
                </div>
              )}
            </div>
          )}
            ...
            <div className="space-y-4 p-6 bg-gray-50 rounded-lg">
              ...
              <div className="flex gap-4 mb-4">
                <button type="button" onClick={() => handleContentChange('pdfUploadType', 'upload')}
                  className={`flex-1 px-4 py-2 rounded-lg border-2 transition-all ${formData.content.pdfUploadType === 'upload' ? 'border-green-600 bg-green-50 text-green-600' : 'border-gray-200 hover:border-gray-300'}`}>
              ...
              {formData.content.pdfUploadType === 'upload' ? (
                ...
                <div>
                  ...
                  <input type="url" value={formData.content.pdfUrl} onChange={(e) => handleContentChange('pdfUrl', e.target.value)}
                    className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-green-600" placeholder="https://example.com/file.pdf" />
                </div>
              )}
            </div>
          )}
          ...
          <div>
            ...
            <input type="number" value={formData.content.duration} onChange={(e) => handleContentChange('duration', parseInt(e.target.value) || 0)} min="0"
              className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-green-600" placeholder="15" />
          </div>
          ...
          <div className="flex gap-3 md:gap-4 pt-6 border-t">
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? (
                ...
                <><Save className="w-5 h-5" /><span>حفظ الدرس</span></>
              )}
            </button>
          </div>
        </form>
